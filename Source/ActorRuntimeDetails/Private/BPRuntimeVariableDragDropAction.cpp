...
FKismetRuntimeVariableDragDropAction::FKismetRuntimeVariableDragDropAction()
	: VariableName(NAME_None)
{
}

UBlueprint* FKismetRuntimeVariableDragDropAction::GetSourceBlueprint() const
{
...
void FKismetRuntimeVariableDragDropAction::HoverTargetChanged()
{
	...
	{
		FMyBlueprintRuntimeItemDragDropAction::HoverTargetChanged();
	}
}

void FKismetRuntimeVariableDragDropAction::GetDefaultStatusSymbol(const FSlateBrush*& PrimaryBrushOut, FSlateColor& IconColorOut, FSlateBrush const*& SecondaryBrushOut, FSlateColor& SecondaryColorOut) const
{
...
FReply FKismetRuntimeVariableDragDropAction::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FKismetRuntimeVariableDragDropAction::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
void FKismetRuntimeVariableDragDropAction::MakeGetter(FNodeConstructionParams InParams)
{
...
void FKismetRuntimeVariableDragDropAction::MakeSetter(FNodeConstructionParams InParams)
{
	check(InParams.Graph);
...
FReply FKismetRuntimeVariableDragDropAction::DroppedOnPanel( const TSharedRef< SWidget >& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{	
		...
		if (VariableProperty && CanVariableBeDropped(VariableProperty, Graph))
		{
			...
			else
			{
				...
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateStatic(&FKismetRuntimeVariableDragDropAction::MakeGetter, NewNodeParams), FCanExecuteAction())
					);
				...
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateStatic(&FKismetRuntimeVariableDragDropAction::MakeSetter, NewNodeParams),
					FCanExecuteAction::CreateStatic(&FKismetRuntimeVariableDragDropAction::CanExecuteMakeSetter, NewNodeParams, VariableProperty ))
					);
...
UStruct* FKismetRuntimeVariableDragDropAction::GetLocalVariableScope() const
{
